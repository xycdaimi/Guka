...
class User:
    ...
    def save_to_json(self, file_path):
        with open("./resource/"+file_path, 'w') as file:
...
def save_history(file_path: str, history: list[conversation.Conversation]):
    ...
    # 保存为.npy文件
    np.save('./resource/historyData/' + file_path, conversation_dicts)
    print(f"历史记录已保存到 {file_path}.npy")


def load_history(file_path: str):
    if os.path.exists('./resource/historyData/' + file_path + '.npy'):
        # 从.npy文件加载字典列表
        conversation_dicts = np.load('./resource/historyData/' + file_path + '.npy', allow_pickle=True)
